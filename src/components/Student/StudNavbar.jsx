...
function StudNavbar(props) {
  ...
  const navToStudClass = () => {
    navigate("/studClass", {
      state: { id: id, token: token, username: username },
    });
  };

  const navToStudAddSubject = () => {
    navigate("/studAddSubject", {
      state: { id: id, token: token, username: username },
    });
  };

  const navToStudWebinar = () => {
    navigate("/studWebinar", {
      state: { id: id, token: token, username: username },
    });
  };

  const navToLogin = () => {
    navigate("/", { state: { id: id, token: token, username: username } });
  };
        ...
        >
          <div className="px-3 py-3 pt-10 flex justify-start items-start h-full text-white">
            <div className=" flex flex-col gap-10">
              ...
              <div onClick={navToLogin} className=" flex gap-3 items-center">
                <h1>
